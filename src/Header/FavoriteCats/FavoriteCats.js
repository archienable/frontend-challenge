...
const FavoriteCats = () => {
	...
	const fetchMoreData = () => {
		setIsLoading(true)
		setNewFavCats([...newFavCats])
	};

	return (
		<div>
			<InfiniteScroll
				className={style.favoriteCatsContainer}
				next={fetchMoreData}
				hasMore={true}
				loader={null}
				dataLength={newFavCats.length}>
					{newFavCats.map(item => {
						...
						return (
							<div className={style.favItem} style={{backgroundImage: `url(${item.url})`}} key={`cats-${item.id}`}>
								<svg className={style.svg} width="48" height="48" viewBox="0 0 48 48" fill="none"
